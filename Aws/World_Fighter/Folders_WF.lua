            ...
            task.spawn(function()
                ...
                while AutoFarm do
                    ...
                    if hrp then
                        ...
                        -- "Check if our current target is still alive" block stays the same,
-- only the search block changes:
if not lastEnemy then
    if #SelectedEnemies == 0 then
        task.wait(1)
    else
        for _, enemy in ipairs(GetAllEnemies()) do
            if enemy:IsA("BasePart") and IsSelectedEnemy(enemy.Name) then
        ...
        if aliveEnemy then
            ...
            WindUI:Notify({Title = "Auto Farm", Content = "Locked onto " .. aliveEnemy.Name, Duration = 1, Icon = "check"})
        else
            task.wait(2)
        end
    end

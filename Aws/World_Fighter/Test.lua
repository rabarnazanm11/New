...
--               TAB: ARISE
-- =====================================================
local Arise_Tab = Window:Tab({Title = "Arise", Icon = "arrow-up-from-dot"})

-- ── State ────────────────────────────────────────────────────────────────────
local ariseUnitIdMap        = {}
local ariseSelectedUnitDisp = nil
local isArising             = false
local AriseToggleRef        = nil
local ariseDelay            = 0.5

-- ── Helpers ──────────────────────────────────────────────────────────────────
local function buildAriseUnitValues()
    ariseUnitIdMap = {}
    local values = {}
    for unitId, unitData in Omni.Data.Inventory.Units do
        local rarity  = unitData.Rarity or "?"
        local baseName = unitData.CustomName or unitData.Name
        local display = baseName .. " [" .. rarity .. "]"

        -- If two units share the same display string, disambiguate with a short id
        if ariseUnitIdMap[display] then
            display = display .. " (" .. string.sub(unitId, 1, 4) .. ")"
        end

        ariseUnitIdMap[display] = unitId
        table.insert(values, display)
    end
    table.sort(values)
    return values
...
local function fireArise(unitId)
    local args = {
        {
            {
                "General", "Units", "Arise",
...
-- ── UI ───────────────────────────────────────────────────────────────────────
local ariseUnitValues   = buildAriseUnitValues()
ariseSelectedUnitDisp   = ariseUnitValues[1]

local AriseUnitDropdown = Arise_Tab:Dropdown({
    Title = "Select Unit",
    Desc = "Shows Name [Rarity] — all inventory units",
    Values = ariseUnitValues,
    Value = ariseUnitValues[1],
    Multi = false,
    AllowNone = false,
    SearchBarEnabled = true,
    Flag = "arise_unit",
    Callback = function(selected)
        ariseSelectedUnitDisp = selected
    end
})
...
Arise_Tab:Button({
    ...
    Justify = "Center", IconAlign = "Left",
    Callback = function()
        local newValues = buildAriseUnitValues()
        ariseSelectedUnitDisp = newValues[1]
        AriseUnitDropdown:Refresh(newValues)
        WindUI:Notify({
            Title = "Arise",
            Content = "Unit list refreshed (" .. #newValues .. " units)",
...
Arise_Tab:Slider({
    Title = "Arise Speed",
    Desc = "Delay between each arise fire",
    IsTooltip = true, IsTextbox = true, Step = 0.1,
    Value = {Min = 0.1, Max = 3, Default = 0.5}, Width = 140,
    Icons = {From = "minus", To = "plus"},
...
AriseToggleRef = Arise_Tab:Toggle({
    Title = "Auto Arise",
    Desc = "Continuously fires Arise on the selected unit until toggled off",
    ...
    Callback = function(state)
        isArising = state
        if isArising then
            if not ariseSelectedUnitDisp then
                WindUI:Notify({
                    Title = "Arise",
                    Content = "Select a unit first!",
                    Duration = 3, Icon = "alert-circle"
                })
                pcall(function() AriseToggleRef:Set(false) end)
                isArising = false
                return
            end
            WindUI:Notify({
                Title = "Arise",
                Content = "Arising: " .. ariseSelectedUnitDisp,
                Duration = 3, Icon = "arrow-up-from-dot"
            })
            task.spawn(function()
                while isArising do
                    local unitId = ariseUnitIdMap[ariseSelectedUnitDisp]
                    if unitId then
                        fireArise(unitId)
                    end
                    task.wait(ariseDelay)
                end
            end)
        end
    end
...
local function FetchServers()
    ...
    for _, server in pairs(data.data) do
        -- At least 2 open slots (buffer in case one fills between fetch and join)
        if server.maxPlayers - server.playing >= 2 then
...
local function HopToNextServer()
    ...
    local jobId = serverQueue[currentIndex]
    ...
    if not ok then
        warn("Server full or gone, skipping... (" .. tostring(err) .. ")")
        task.wait(0.5)
        HopToNextServer() -- try the next one immediately
    end
end
...
Players.LocalPlayer.CharacterAdded:Connect(function()
    ...
    if bossPart then
        ...
        WindUI:Notify({Title = "Boss Found!", Content = bossName .. " is here!", Duration = 5, Icon = "check"})
    else
...
local Global_Boss_Section = GloBalBosses:Section({
    Title = "Global Boss Hunter",
    Icon = "swords",
    Box = true, BoxBorder = true, Opened = true,
    FontWeight = Enum.FontWeight.ExtraBold,
    TextSize = 18,
...
Global_Boss_Section:Slider({
    ...
    Width = 140,
    Icons = { From = "sfsymbols:minusCircleFill", To = "sfsymbols:plusCircleFill" },
    Callback = function(value)
        HopDelay = value
    end
})

Global_Boss_Section:Toggle({
    Title = "Auto Teleport to Boss",
    Desc = "Keeps teleporting you to the boss on this server",
    Callback = function(state)
        TeleportToggle_On = state

        if BossHeartbeatConn then
            BossHeartbeatConn:Disconnect()
            BossHeartbeatConn = nil
        end
        ...
        BossHeartbeatConn = RunService.Heartbeat:Connect(function(dt)
            if not TeleportToggle_On then return end
...
Global_Boss_Section:Toggle({
    ...
    Callback = function(state)
        ...
        -- Check current server first before hopping
        local bossPart, bossName = SearchBoss()
        if bossPart then
            warn("Boss already here: " .. bossName)
            WindUI:Notify({Title = "Boss Found!", Content = bossName .. " is already here!", Duration = 5, Icon = "check"})
            return
        end

...
local function isDragMovement(input)
	return input.UserInputType == Enum.UserInputType.MouseMovement
		-- TouchPan doesn't exist - touch dragging handled via Touch + position changes
end
...
local function createOptionHolder(holderTitle, parent, parentTable, subHolder)
	...
	if not subHolder then
		round = library:Create("ImageLabel", {
			...
			ScaleType = Enum.ScaleType.Slice,			SliceCenter = Rect.new(100, 100, 100, 100),
			SliceScale = 0.04,			Parent = parentTable.main
		})
	end
	...
	local layout = library:Create("UIListLayout", {
		...
		parentTable.main.Size = #parentTable.options > 0 and parentTable.open and UDim2.new(0, 230, 0, layout.AbsoluteContentSize.Y + size) or UDim2.new(0, 230, 0, size)	end)
	...
	if not subHolder then
			...
			if isPrimaryPress(input) then
				...
				if input.UserInputType == Enum.UserInputType.Touch then
					input.Changed:Connect(function(prop)
						if prop == "UserInputState" and isRelease(input) then
							dragging = false
						end
					end)
				end
		...
		title.InputChanged:connect(function(input)
			if dragging and isDragMovement(input) then
				dragInput = input
			end
		end)
		
		title.InputEnded:connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or isRelease(input) then
				dragging = false
			end
		end)
	end
	...
	function parentTable:SetTitle(newTitle)
		title.Text = tostring(newTitle)	end
...
local function createToggle(option, parent)
	...
	main.InputBegan:connect(function(input)
		...
		if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement then
			inContact = true
	...
	main.InputEnded:connect(function(input)
		if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement then
			inContact = false			if not option.state then
		...
		if state then
			...
			if inContact and not isTouchDevice then
...
local function createButton(option, parent)
	...
	local round = library:Create("ImageLabel", {
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.new(0.5, 0, 0.5, 0),		Size = UDim2.new(1, -12, 1, -10),
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(40, 40, 40),
		ScaleType = Enum.ScaleType.Slice,
		SliceCenter = Rect.new(100, 100, 100, 100),
		SliceScale = 0.02,		Parent = main
	})
...
local function createBind(option, parent)
	...
	local main = library:Create("TextLabel", {		LayoutOrder = option.position,
		...
		Font = Enum.Font.Gotham,
		TextColor3 = Color3.fromRGB(255, 255, 255),
		TextXAlignment = Enum.TextXAlignment.Left,		Parent = parent.content
	})
	...
	main.InputEnded:connect(function(input)
		if isPrimaryPress(input) and not binding then
			binding = true
			bindinput.Text = "..."
			tweenService:Create(round, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(255, 65, 65)}):Play()
		end		if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement then
			inContact = false
			if not binding then
				tweenService:Create(round, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(40, 40, 40)}):Play()
			end
	...
	inputService.InputBegan:connect(function(input)		if inputService:GetFocusedTextBox() then return end
	...
	function option:SetKey(key)
		...
		library.flags[self.flag] = self.key		if string.match(self.key, "Mouse") then
			bindinput.Text = string.sub(self.key, 1, 5) .. string.sub(self.key, 12, 13)
		else
			bindinput.Text = self.key
		end
		tweenService:Create(round, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = inContact and not isTouchDevice and Color3.fromRGB(60, 60, 60) or Color3.fromRGB(40, 40, 40)}):Play()
...
local function createSlider(option, parent)
	...
	if option.min >= 0 then
		fill.Size = UDim2.new((option.value - option.min) / (option.max - option.min), 0, 1, 0)	else
	...
	main.InputBegan:connect(function(input)
		if isPrimaryPress(input) then
			...
			sliding = true			option:SetValue(option.min + ((input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X) * (option.max - option.min))
			if input.UserInputType == Enum.UserInputType.Touch then
				input.Changed:Connect(function(prop)
					if prop == "UserInputState" and isRelease(input) then
						sliding = false
	...
	main.InputEnded:connect(function(input)
		...
		if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement then
			inContact = false
			inputvalue:ReleaseFocus()			if not sliding then
				tweenService:Create(fill, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(60, 60, 60)}):Play()
				tweenService:Create(circle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 0, 0, 0), ImageColor3 = Color3.fromRGB(60, 60, 60)}):Play()
			end
		end
	...
	function option:SetValue(value)		value = round(value, option.float)
...
local function createList(option, parent, holder)
	...
	local listvalue = library:Create("TextLabel", {		Position = UDim2.new(0, 12, 0, 20),
	...
	library:Create("UIPadding", {
		PaddingTop = UDim.new(0, 6),		Parent = content
	})
	...
	round.InputBegan:connect(function(input)
		...
		if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if not option.open then
				tweenService:Create(round, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(60, 60, 60)}):Play()			end
		end
	end)
	...
	function option:AddValue(value)
		valueCount = valueCount + 1		local label = library:Create("TextLabel", {
			ZIndex = 3,
		...
		label.InputEnded:connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or isRelease(input) then
				clicking = false
				tweenService:Create(label, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = inContact and not isTouchDevice and Color3.fromRGB(20, 20, 20) or Color3.fromRGB(30, 30, 30)}):Play()
			end			if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement then
				inContact = false
				if not clicking then
	...
	function option:Close()
		...
		wait(0.3)		if not self.open then
...
local function createBox(option, parent)
	...
	local outline = library:Create("ImageLabel", {		Position = UDim2.new(0, 6, 0, 4),
		Size = UDim2.new(1, -12, 1, -10),
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(60, 60, 60),
	...
	local inContact
	local focused
	main.InputBegan:connect(function(input)		if isPrimaryPress(input) then
	...
	function option:SetValue(value, enter)		library.flags[self.flag] = tostring(value)
		self.value = tostring(value)
...
local function createColorPickerWindow(option)
	...
	inputService.InputChanged:connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseMovement then
			if editinghue then
				local X = (Input.Position.X - option.hue.AbsolutePosition.X) / option.hue.AbsoluteSize.X
				X = math.clamp(X, 0, 0.995)
				option:updateVisuals(Color3.fromHSV(1 - X, sat, val))			end
		end
	end)
	...
	option.satvalSlider = library:Create("Frame", {
		ZIndex = 3,
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.new(sat, 0, 1 - val, 0),		Size = UDim2.new(0, 4, 0, 4),
		Rotation = 45,
		BackgroundTransparency = 1,
	...
	inputService.InputChanged:connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseMovement then
			if editingsatval then
				local X = (Input.Position.X - option.satval.AbsolutePosition.X) / option.satval.AbsoluteSize.X				local Y = (Input.Position.Y - option.satval.AbsolutePosition.Y) / option.satval.AbsoluteSize.Y
				X = math.clamp(X, 0.005, 1)
				Y = math.clamp(Y, 0, 0.995)
				option:updateVisuals(Color3.fromHSV(hue, X, 1 - Y))
			end
	...
	option.resetColor = library:Create("ImageLabel", {		ZIndex = 3,
	...
	option.resetColor.InputBegan:connect(function(Input)
		if isPrimaryPress(Input) and not rainbowEnabled then
			previousColors = {originalColor}			option:SetColor(originalColor)
		end
	end)
	...
	option.setColor = library:Create("ImageLabel", {
		...
		ImageColor3 = Color3.fromRGB(20, 20, 20),
		ScaleType = Enum.ScaleType.Slice,
		SliceCenter = Rect.new(100, 100, 100, 100),		SliceScale = 0.02,
		Parent = option.mainHolder
	})
	...
	option.rainbow = library:Create("ImageLabel", {		ZIndex = 3,
	...
	option.rainbow.InputBegan:connect(function(Input)		if isPrimaryPress(Input) then
...
local function createColor(option, parent, holder)
	option.main = library:Create("TextLabel", {
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, 31),
		BackgroundTransparency = 1,		Text = " " .. option.text,
		TextSize = 17,
		Font = Enum.Font.Gotham,
		TextColor3 = Color3.fromRGB(255, 255, 255),
		TextXAlignment = Enum.TextXAlignment.Left,
		...
		if isPrimaryPress(input) then
			...
			for _,object in next, option.mainHolder:GetDescendants() do
				if object:IsA"TextLabel" then
					tweenService:Create(object, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()				elseif object:IsA"ImageLabel" then
	...
	function option:Close()
		...
		delay(0.3, function()
			if not self.open then				self.mainHolder.Visible = false
			end 
		end)
	end
end
...
local function getFnctions(parent)
	...
	function parent:AddBind(option)
		...
		option.type = "bind"
		option.position = #self.options
		option.flag = option.flag or option.text		library.flags[option.flag] = option.key
		table.insert(self.options, option)
		return option
	end
	...
	function parent:AddList(option)
		...
		option.callback = typeof(option.callback) == "function" and option.callback or function() end		option.open = false
		option.type = "list"
		option.position = #self.options
		option.flag = option.flag or option.text
		library.flags[option.flag] = option.value
...
function library:Init()
	...
	if syn and syn.protect_gui then
		...
		self.base.Parent = game:GetService("CoreGui")	elseif gethui then
...
function library:Close()
	...
	self.cursor.Visible = self.open and not isTouchDevice
...
inputService.InputBegan:connect(function(input)
	if isPrimaryPress(input) then
		if library.activePopup then
			if input.Position.X < library.activePopup.mainHolder.AbsolutePosition.X or input.Position.Y < library.activePopup.mainHolder.AbsolutePosition.Y then				library.activePopup:Close()
			end
...
inputService.InputChanged:connect(function(input)
	if not isTouchDevice and input.UserInputType == Enum.UserInputType.MouseMovement and library.cursor then
